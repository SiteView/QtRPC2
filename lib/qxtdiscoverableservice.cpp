...
void QxtDiscoverableServicePrivate::socketData()
{
        ...
        if (test)
        {
            service = new _DNSServiceRef_t;
            //test(service);
            //test(qxt_d.)
        }
        //DNSServiceProcessResult(service);
        qDebug() << "socket data end";
...
void QxtDiscoverableService::registerService(bool noAutoRename)
{
	...
	DNSServiceErrorType err;
        ...
        QLibrary *lib = new QLibrary("dnssd");
        //QLibrary *lib = new QLibrary("E:\\cyj\\networkmap\\QtRpc2\\lib\\dnssd.dll");
        lib->load();
        if (lib->isLoaded())
        {
            qDebug() << "load success register";
        }
        else
        {
            qDebug() << "load failed";
        ...
        typedef DNSServiceErrorType (*Fun)(DNSServiceRef *,DNSServiceFlags,unsigned int,const char *,const char*,const char*, const char *,uint16_t,uint16_t, const void *, DNSServiceRegisterReply, void*);
        Fun test = (Fun)lib->resolve("DNSServiceRegister");

        if (test)
        {
            qDebug() << "register service";
            qDebug() << "service:" << &(qxt_d().service);
...
//                }
                ...
                if (test1)
                {
                    qDebug() << "DNSService Ref SockeFD";

                    qxt_d().notifier = new QSocketNotifier(test1(qxt_d().service), QSocketNotifier::Read, this);
...
void QxtDiscoverableService::resolve(bool forceMulticast)
{
	...
	qxt_d().service = 0;
        ...
        typedef DNSServiceErrorType (*Fun)(DNSServiceRef *,DNSServiceFlags,unsigned int,const char *,const char *,const char *,DNSServiceResolveReply, void *);
        Fun test = (Fun)lib->resolve("DNSServiceResolve");
